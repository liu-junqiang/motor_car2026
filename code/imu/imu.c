...
void gesture_systematic_error_calc(void)
{
    ...
    {
        // 积分
        imu_temp->gyro_integral_z += (float)imu660rc_gyro_z;
...
void imu_get(void)
{
    Gyro = imu660rc_gyro_transition(imu660rc_gyro_y);//将 IMU660RC 陀螺仪数据转换为实际物理数据
    //低通滤波
//    Gyro = Gyro * alpha + (1 - alpha) * Gyro_last;
//    Gyro_last = Gyro;

    Pitch = -imu660rc_pitch;
    if(imu660rc_yaw>180)
    {
        Yaw = imu660rc_yaw - 360;
    }
    else
        Yaw = imu660rc_yaw;
}

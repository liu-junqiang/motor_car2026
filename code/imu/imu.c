...
extern signed short  imu660rc_acc_x, imu660rc_acc_z;            //加速度传感器原始数据
extern signed short imu660rc_gyro_y,imu660rc_gyro_z;         //陀螺仪原始数据
...
void Euler(void) {
    ...
    //将 imu660rc 加速度计数据转换为实际物理数据将，单位为 g(m/s^2)
    ay = -imu660rc_acc_transition(imu660rc_acc_x);
    az = imu660rc_acc_transition(imu660rc_acc_z);

    Gyro_x_temp = imu660rc_gyro_transition(imu660rc_gyro_y);//把陀螺仪读取到的角速度转换成实际角速度付给Gyro_x_temp
        Gyro_x = Alpha * Gyro_x_temp + (1-Alpha) * Gyro_x_last;
        Gyro_x_last = Gyro_x;

        zhou=imu660rc_gyro_transition(imu660rc_gyro_y);//比较滤波效果
    //将 imu660rc 陀螺仪数据转换为实际物理数据，单位为°/s

    Gyro_z = imu660rc_gyro_transition(imu660rc_gyro_z);
